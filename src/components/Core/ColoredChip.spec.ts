...
const value = 'foo'
const fooBackgroundRgb = 'rgb(63, 98, 18)'
const fooTextRgb = 'rgb(217, 249, 157)'
...
describe('ColoredChip.vue', () => {
  it('should render chip with random color and value', () => {
    ...
    expect(chip.attributes().style).toContain(`background-color: ${fooBackgroundRgb}`)
  ...
  it('should convert named color', () => {
    ...
    const el: HTMLSpanElement = chip.element
    expect(el.nodeName).eq('DIV')
    expect(chip.attributes().style).toContain(`color: ${fooTextRgb}`)
  })
})
